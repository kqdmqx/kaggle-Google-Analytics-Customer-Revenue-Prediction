...
def main(nrows=None):
    ...
    # test.to_csv('extracted_fields_test.gz', compression='gzip', index=False, encoding="utf-8")
    # train.to_csv('extracted_fields_train.gz', compression='gzip', index=False, encoding="utf-8")

    save_dataframe("./data/extracted_fields_test", test) 
    logger.info("save test done.")
    save_dataframe("./data/extracted_fields_train", train)
    logger.info("save train done.")
